...
public interface FundWalletRepository extends JpaRepository<FundWalletEntity, Long> {
    ...
    @Query(value = "SELECT " +
            "r.symbol AS symbol, SUM(f.amount) AS totalAmount, SUM(f.price) AS totalPrice" +
            " FROM fund_wallet f, fund_scrap r WHERE f.fund_id = r.id AND f.profile_id = ?1 GROUP BY f.fund_id, r.symbol", nativeQuery = true)
